...
const char *WINDOWTITLE = { "Project_2, Weijie Mo" };
...
GLUI_HSlider * slider1;
GLUI_HSlider * slider2;
GLUI_HSlider * slider3;
GLUI_HSlider * slider4;
GLUI_HSlider * slider5;
GLUI_HSlider * slider6;
...
Buttons(int id)
{
	...
	case RESET:
		Reset();
		slider1->set_slider_val(SLowHigh[0], SLowHigh[1]);
		slider2->set_slider_val(XLowHigh[0], XLowHigh[1]);
		slider3->set_slider_val(YLowHigh[0], YLowHigh[1]);
		slider4->set_slider_val(ZLowHigh[0], ZLowHigh[1]);
		slider5->set_slider_val(RLowHigh[0], RLowHigh[1]);
		slider6->set_slider_val(GLowHigh[0], GLowHigh[1]);
...
InitGlui()
{
	...
	slider1 = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, SLowHigh, S, (GLUI_Update_CB)Sliders);
	slider1->set_float_limits(SLowHigh[0], SLowHigh[1]);
	slider1->set_slider_val(SLowHigh[0], SLowHigh[1]);
	slider1->set_w(SLIDERWIDTH);
	sprintf(str, SFORMAT, SLowHigh[0], SLowHigh[1]);
	...
	slider2 = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, XLowHigh, X, (GLUI_Update_CB)Sliders);
	slider2->set_float_limits(XLowHigh[0], XLowHigh[1]);
	slider2->set_slider_val(XLowHigh[0], XLowHigh[1]);
	slider2->set_w(SLIDERWIDTH);
	sprintf(str, XFORMAT, XLowHigh[0], XLowHigh[1]);
	XLabel = Glui->add_statictext_to_panel(rollout, str);
	Glui->add_separator_to_panel(rollout);

	slider3 = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, YLowHigh, Y, (GLUI_Update_CB)Sliders);
	slider3->set_float_limits(YLowHigh[0], YLowHigh[1]);
	slider3->set_slider_val(YLowHigh[0], YLowHigh[1]);
	slider3->set_w(SLIDERWIDTH);
	sprintf(str, YFORMAT, YLowHigh[0], YLowHigh[1]);
	YLabel = Glui->add_statictext_to_panel(rollout, str);
	Glui->add_separator_to_panel(rollout);

	slider4 = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, ZLowHigh, Z, (GLUI_Update_CB)Sliders);
	slider4->set_float_limits(ZLowHigh[0], ZLowHigh[1]);
	slider4->set_slider_val(ZLowHigh[0], ZLowHigh[1]);
	slider4->set_w(SLIDERWIDTH);
	sprintf(str, ZFORMAT, ZLowHigh[0], ZLowHigh[1]);
	ZLabel = Glui->add_statictext_to_panel(rollout, str);
	Glui->add_separator_to_panel(rollout);

	slider5 = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, RLowHigh, R, (GLUI_Update_CB)Sliders);
	slider5->set_float_limits(RLowHigh[0], RLowHigh[1]);
	slider5->set_slider_val(RLowHigh[0], RLowHigh[1]);
	slider5->set_w(SLIDERWIDTH);
	sprintf(str, RFORMAT, RLowHigh[0], RLowHigh[1]);
	RLabel = Glui->add_statictext_to_panel(rollout, str);
	Glui->add_separator_to_panel(rollout);

	slider6 = Glui->add_slider_to_panel(rollout, true, GLUI_HSLIDER_FLOAT, GLowHigh, G, (GLUI_Update_CB)Sliders);
	slider6->set_float_limits(GLowHigh[0], GLowHigh[1]);
	slider6->set_slider_val(GLowHigh[0], GLowHigh[1]);
	slider6->set_w(SLIDERWIDTH);
	sprintf(str, GFORMAT, GLowHigh[0], GLowHigh[1]);
...
InitGraphics()
{
	...
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				float tempx = -1.0 + 2.0 * i / (NX - 1);
				float tempy = -1.0 + 2.0 *(j) / (NY - 1);
				float tempz = -1.0 + 2.0 *((k)) / (NZ - 1);
				float t = Temperature(tempx, tempy, tempz);
